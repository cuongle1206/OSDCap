...
def compute_trans_loss(q, qgt):
    return loss_fn(q[...,:3], qgt[...,:3])
...
# def compute_fric_loss(c, p_moved):
#     fric_loss       = (((c[:,0:1]>0.5).float() * p_moved[:,[3,4]]).sum() + ((c[:,1:2]>0.5).float() * p_moved[:,[8,9]]).sum())
