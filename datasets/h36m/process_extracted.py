...
def get_parse():
    parser = argparse.ArgumentParser(description='Generation of ground truth data for Human 3.6M') 
    parser.add_argument("-p", "--data-path", type=str,
                        default="/mimer/NOBACKUP/groups/alvis_cvl/cuole/datasets/h36m/extracted",
                        help="Specify the absolute path to your data folder")
    return parser
...
def process_view(out_dir, subject, action, subaction, camera):
    ...
    subj_dir        = path.join(args.data_path, subject)
    ...
    # with pycdf.CDF(path.join(subj_dir, 'Poses_D3_Angles_mono', base_filename + '.cdf')) as cdf:
    #     angles_3d = np.array(cdf['Pose'])
    #     angles_3d = angles_3d.reshape(angles_3d.shape[1], 26, 3)
    base_filename2 = metadata.get_base_filename_2(subject, action, subaction)
    with pycdf.CDF(path.join(subj_dir, 'Poses_D3_Angles', base_filename2 + '.cdf')) as cdf:
        angles_3d_gt = np.array(cdf['Pose'])
        angles_3d_gt = angles_3d_gt.reshape(angles_3d_gt.shape[1], -1, 3)
    ...
    return {
        ...
        # 'angles' : angles_3d[frame_indices],
        'angles' : angles_3d_gt[frame_indices],
